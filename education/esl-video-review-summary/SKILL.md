...
Design and tone follow the `esl-writing-analysis` design system (Notion-inspired: Inter, warm whites, whisper borders, 5-color legend, fix cards, strengths box, tips grid, closing). If that skill is available in your setup, read it with skill_view for the full system; otherwise reproduce the same minimal card layout.

## User preferences (hard rules)

- **NEVER embed screenshots from the video.** Word-timestamps are for STRUCTURE ONLY: locate where each task/correction happens and split the document into sections.
...
- Filename: `{output-dir}/{student}-{topic}-{YYYYMMDD}.html` (e.g. `~/esl-analysis/`), never overwrite existing files.
- Run an anti-AI-ism pass on teacher prose; no audit footer.

## Workflow (verified on a 16-min, 1.4 GB screen recording)
...
AAC stream copy is instant (~13 MB for 16 min). Any recent ffmpeg works; no re-encode needed for Deepgram.

### 2. Transcribe via Deepgram — raw body, NOT multipart

Some sandboxed/proxied environments corrupt multipart uploads (400 "corrupt data"). Send raw bytes with a Content-Type header instead:

```python
import os, requests
key = os.environ["DEEPGRAM_API_KEY"]          # never hardcode; export it or load a local .env
...
- Practice tips: test-english links at the student's level (e.g. for an A2 student: prepositions-of-movement, past-continuous-past-simple, comparative-superlative-adjectives-adverbs, or the general /grammar-points/a2 index). Verify URL slugs before linking.
...
## Keeping a public copy in sync

This skill is published in the `itsfedor/hermes-skills` collection. After an update, push the change (repo layout: `<category>/<skill-name>/SKILL.md` plus a row in the repo README table) and verify the push with the contents API — confirm the file list and sizes, not just the push exit code.

Authenticate with a credential helper or `gh auth login`. **Never put a token in the remote URL** — `https://x-access-token:<TOKEN>@github.com/...` writes the secret into `.git/config` in cleartext.

**Pitfall — auto_init README conflict:** creating a repo via the API with `auto_init: true` leaves a remote README commit, so the first push is rejected (non-fast-forward). Fix: `git pull --rebase origin main`, resolve the README conflict (keep the local one), `git add` + `git commit`, `git rebase --continue`, then push.

## Pitfalls

- **ASR name mangling:** student and character names get misheard (a student's name may come out as two different spellings; rope→Rob, holy→Holly, Jane→Jenny in a series). Resolve from context (who appears repeatedly, who fits the story), and explicitly tell the user which names you assumed so they can correct.
